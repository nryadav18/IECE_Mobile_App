...
export default function CreateEventForm({ onEventCreated }) {
  ...
  const pickMedia = async () => {
    let result = await ImagePicker.launchImageLibraryAsync({
      ...
      aspect: [16, 9],
      quality: 0.8,
      videoMaxDuration: 60, // Enforce some limit
    });

    if (!result.canceled) {
      // Filter out > 20MB files
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showAlert('Success', 'Event published successfully!', 'success');
      ...
      showAlert('Error', 'Failed to publish event.', 'error');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
      ...
      <Text style={[styles.title, { color: theme.colors.textPrimary }]}>Publish New Event</Text>
      ...
      <TouchableOpacity 
        ...
        disabled={uploading}
      >
        {uploading ? <ActivityIndicator color="#FFF" /> : <Text style={styles.submitBtnText}>Publish Event</Text>}
      </TouchableOpacity>
